...
public class CachePoisoningScanWorker extends ScanWorker {
    ...
    public CachePoisoningScanWorker(MontoyaApi api, HttpRequestResponse requestResponse, List<String> testDelimitersList, boolean subHosts){
        ...
        for (Server serv : servers.values()){
            ...
            if (serv.getKeyNormalization()[Server.DOT_SEGMENT]){
                for (String delim : discrepancyOriginDelimiters){
                    for (HttpRequestResponse reqResp : serv.getStaticRequest()){
                        ...
                        api.http().sendRequest(setPathSuffix(reqResp.request(), delim+"/../"+random));
                        api.http().sendRequest(setPathSuffix(reqResp.request(), delim+"/../"+random));
                        HttpRequestResponse testResp = api.http().sendRequest(reqResp.request().withPath(Server.removeLastSegment(reqResp.request().path())+"/"+random));
            ...
            if (serv.getOriginNormalization()[Server.DOT_SEGMENT]){
                for (String delim : discrepancyKeyDelimiters){
                    for (HttpRequestResponse reqResp : serv.getStaticRequest()){
                        String random = Server.randomNonce(5);
                        api.http().sendRequest(reqResp.request().withPath("/"+random+delim+"/.."+reqResp.request().path()));
                        api.http().sendRequest(reqResp.request().withPath("/"+random+delim+"/.."+reqResp.request().path()));
                        HttpRequestResponse testResp = api.http().sendRequest(reqResp.request().withPath("/"+random));
                        if (compareResp(testResp.response(), reqResp.response())){
